...
const MUTATIONS = [
  ...
  { id: 'M14 password grant re-enabled on an SPA', run: (d) => { cl(d, 'albaraka-backoffice-web').directAccessGrantsEnabled = true; }, expect: 'direct access grants must be' },
  { id: 'M14b bridge loses its DAG', run: (d) => { cl(d, 'albaraka-assistant-bridge').directAccessGrantsEnabled = false; }, expect: 'direct access grants must be' },
