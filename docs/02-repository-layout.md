...
│   ├── asyncapi.yaml                      (SSE + internal job events; checked by tools/asyncapi-lint)
│   ├── db/
...
├── deploy/                                 ← local/dev topology (docs/12 §2.2)
│   ├── docker-compose.yml                 (data plane + app profile; checked by tools/compose-lint)
│   ├── README.md                          (what runs in Phase 0, what is frozen for Phase 1)
│   ├── postgres/init/                     (10-keycloak-db.sql — first-init only)
│   ├── runbooks/                          (13 runbooks, docs/12 §8)
│   ├── docker/                            (Dockerfiles: server, web, keycloak theme)   ← Phase 1
│   ├── k8s/                               (manifests / Helm chart — Phase 5)
│   └── nginx/                             (edge config, CORS, rate limits)              ← Phase 1
...
│   ├── asyncapi-lint/                     (4 groups · 31 mutations)
│   ├── compose-lint/                      (6 groups · 33 mutations)
│   ├── spike/                             (provider & dependency smoke tests, no build required)
...
├── .gitignore
├── .env.example                           (every env var the gates and compose consume)
├── .editorconfig  .gitattributes  Makefile  CHANGELOG.md                                ← Phase 1
```
