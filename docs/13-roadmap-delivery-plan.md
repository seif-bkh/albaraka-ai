...
| **Tasks** | LangChain pipeline in `rag_assistant` (`langchain-groq` · `langchain-google-genai` · `langchain-postgres` PGVectorStore) · Arabic/French normaliser + light stemmer + FTS config · Derja map · chunker (heading-aware, per-content-type policies) · ingestion jobs (upload + parse + chunk + embed, worker role) · query transform (rewrite, glossary expansion, cross-lingual fallback) · RRF fusion · LLM reranker · context assembler with token budget + MMR · generator with the answer contract · SSE frames (accepted/status/sources/token/answer/refusal/error/done) · citation model + source panel · input/output guardrails (lexicon, PII regex, grounding check, numeric validator) · refusal templates REF-01/03/04/05 · semantic cache · `retrieval_trace` persistence in Spring · golden gate `rag-assistant/tests/test_golden.py` (8 cases) + `tools/eval-lint` |
